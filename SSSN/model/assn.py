...
from .base.feature import extract_feat_vgg, extract_feat_res, extract_feat_conformer
from .base.correlation import Correlation
from .learner import ASSNLearner
...
class ASSN(nn.Module):
    def __init__(self, cfg, use_original_imgsize):
        ...
        if self.backbone_type == 'vgg16':
            ...
            self.backbone = Conformer_small_patch16()
            self.feat_ids = list(range(4, 13))
            self.extract_feats = extract_feat_conformer
            nbottlenecks = [4, 4, 3, 1]
        else:
        ...
        self.backbone.eval()
        self.hpn_learner = ASSNLearner(cfg, inch=list(reversed(nbottlenecks[-3:])))
    ...
    def resize_feats_transformer(self, feats):

        resized_feats = []

        for feat in feats:
            
            resized_feats.append(F.interpolate(feat, size=32, mode='bilinear', align_corners=True))
        
        return resized_feats    

    def forward(self, query_img, support_img, support_mask):
        with torch.no_grad():
            query_feats, query_feats_transformer = self.extract_feats(query_img, self.backbone, self.feat_ids, self.bottleneck_ids, self.lids)
            support_feats, support_feats_transformer = self.extract_feats(support_img, self.backbone, self.feat_ids, self.bottleneck_ids, self.lids)
            query_feats = self.resize_feats(query_feats, self.stack_ids)
            support_feats = self.resize_feats(support_feats, self.stack_ids)
            
            query_feats_transformer = self.resize_feats_transformer(query_feats_transformer)
            support_feats_transformer = self.resize_feats_transformer(support_feats_transformer)

            if self.cfg.TRAIN.MASK:
                support_feats = self.mask_feature(support_feats, support_mask.clone())
                # support_feats_transformer = self.mask_feature(support_feats_transformer, support_mask.clone())
            corr = Correlation.multilayer_correlation(query_feats[-self.stack_ids[-1]:], support_feats[-self.stack_ids[-1]:], self.stack_ids)
            corr_transformer = Correlation.transformer_correlation(query_feats_transformer[-self.stack_ids[-1]:], support_feats_transformer[-self.stack_ids[-1]:])

        logit_mask = self.hpn_learner(corr, self.stack_feats(query_feats), support_mask, corr_transformer)
